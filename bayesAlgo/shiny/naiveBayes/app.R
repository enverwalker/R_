...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        sidebarPanel(
            wellPanel(
                tags$h2("Первый класс", style = "color: blue"),
                sliderInput(
                    inputId = "n",
            ...
            wellPanel(
                tags$h2("Второй класс", style = "color: orange"),
                sliderInput(
                    inputId = "m",
                    label = "Количество элементов:",
                    min = 100,
...
server <- function(input, output) {
    output$plot = renderPlot({
        ...
        for (i in x) {
            for (j in y) {
                res1 <- naiveBayes(c(i, j), m1, d1, 0.1, 1)
                res2 <- naiveBayes(c(i, j), m2, d2, 0.1, 1)
                color <- ifelse(res1 > res2, colors[1], colors[2])
                
                #cat(res1, " ", res2, " ", color, "\n")
                points(i, j, pch = 10, col = color)
            }
        }
